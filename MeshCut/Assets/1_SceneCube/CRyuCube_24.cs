...
{
    Vector3[] mVertices = null;     //정점들의 정보
    int[] mIndex = null;                //인덱스들의 정보
    ...
    void Start()
    {
        ...
        tMaterials[0] = new Material(Shader.Find("Standard"));//셰이더를 검색하여 찾아 적용한다
        ...
        Vector3[] tNormals = new Vector3[mVertices.Length];//정점 갯수만큼 준비 
        for(int ti = 0; ti<mVertices.Length;ti+=3)
        {
            //법선벡터를 구해, 한 평면에 속한 정점 세개의, 정점마다의 법선벡터로 설정함
    ...
    void Update()
    {
        ...
        //tMtl.name = "ryu_0";
        //tMtl.color = Color.blue;//Color.white;//색상설정
    }
}
