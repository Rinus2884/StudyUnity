...
public class CRyuDeformMesh : MonoBehaviour
{
    ...
    Vector3[] mOriginVerts = null;      //게임오브젝트의 메쉬에서 얻어온 정점들의 정보
    Vector3[] mNewVerts = null;         //변형을 가해 새롭게 만들어진 메쉬의 정점들의 정보
    ...
    private void OnGUI()
    {
        //임의의 정점들의 이동변환을 '간단하게 테스트'해본다.
       if( GUI.Button(new Rect(0f, 0f, 100f, 100), "Test Translate \nAny Vertices") )
        {
            mMeshFilter = this.GetComponent<MeshFilter>();

            mOriginVerts = mMeshFilter.mesh.vertices;//메쉬필터로부터 정점의 정보들을 얻는다

            //기존 정점 의 갯수만큼 새로운 정점의 갯수를 확보한다.
            mNewVerts = new Vector3[mOriginVerts.Length];

            //임의의 조건을 만족하는 정점만 변형(이동변환)해본다.
        ...
        //force를 적용하여 변형
        if (GUI.Button(new Rect(200f, 0f, 100f, 100), "Apply Force"))
        {
            mMeshFilter = this.GetComponent<MeshFilter>();
            ...
            for (int ti = 0; ti < mOriginVerts.Length; ++ti)
            {
                ...
                Vector3 tB = mOriginVerts[ti];          //목적지점: 메쉬의 임의의 정점의 위치 정보
                ...
                float tPower = tRadius * 2f * mpForceObj.gameObject.transform.localScale.x;
        ...
        //force를 적용하여 변형
        if (GUI.Button(new Rect(300f, 0f, 100f, 100), "Apply Force Back"))
        {
            mMeshFilter = this.GetComponent<MeshFilter>();
